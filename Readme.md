...
Windowsでは`Codetta Composer.cmd`をダブルクリックします。起動時には結果が`10`になるv0.2受入サンプルが開き、構造編集ボタンをすぐ試せます。別の`.codetta`ファイルはランチャーへドラッグ＆ドロップして開けます。右側のPythonペインにv0.2コードを入力しても、検証後に通常の楽譜へ再配置されます。
